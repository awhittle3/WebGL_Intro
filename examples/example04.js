...
function main() {
    ...
    var state = {
        colour: vec4.fromValues(0.5, 0.5, 0.5, 1.0)
    };

    // Hook up the button
    const button = document.querySelector("#colourButton");
    ...
    const vsSource =
        ...
        void main() {
            ...
            // Mix the attribute colour and our uniform colour
            oColor = mix(aColor, uColor, 0.5);
        }
        `;
...
function drawScene(gl, deltaTime, programInfo, buffers, state) {
    ...
    {
        // Set our uniform colour
        gl.uniform4fv(programInfo.uniformLocations.colour, state.colour);
 ...
 */
function initTriangleBuffers(gl, programInfo) {
    ...
    return {
        ...
        attributes: {
            position: initTrianglePositionAttribute(gl, programInfo, positions),
            colour: initTriangleColourAttribute(gl, programInfo, colours),
        },
        numVertices: 3
    };
}

function initTrianglePositionAttribute(gl, programInfo, positionArray) {
...
function initTriangleColourAttribute(gl, programInfo, colourArray) {
